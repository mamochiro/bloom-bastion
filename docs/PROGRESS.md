...
**Gate (last):** `bun run check` green (Biome 69 · tsc strict · Vitest 112/112 · build) + 5× `--sequence.shuffle` clean. Bundle ≈234KB gz (app 69 + pixi 160 + react 4; budget <400KB). SVG art ships as separate files (`dist/sprites/`), NOT in the JS bundle. Pipeline: 9/10 systems real (only Animation, slot 8, is a stub). Content: 2 towers (Blossom, Stormcloud), 2 enemies (Grub, Snail), 3 waves.
