...
- [ ] Difficulty HP/speed multipliers (§6.5 Casual/Hardcore) — Normal 1.0× hardcoded; apply when difficulty select lands
...
**Gate (last):** `bun run check` green (Biome 68 · tsc strict · Vitest 94/94 · build) + 5× `--sequence.shuffle` clean. Bundle ≈233.5KB gz (app 69 + pixi 160 + react 4; budget <400KB). SVG art ships as separate files (`dist/sprites/`), NOT in the JS bundle. Pipeline: 9/10 systems real (only Animation, slot 8, is a stub).
