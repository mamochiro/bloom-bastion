#!/usr/bin/env bash
# Bloom Bastion Multi-Agent
#
# Usage:
#   ./scripts/multiagent.sh                          # orchestrator only
#   ./scripts/multiagent.sh engine                   # orchestrator + engine
#   ./scripts/multiagent.sh engine gameplay          # + gameplay
#   ./scripts/multiagent.sh engine gameplay ui qa    # all 5 agents
#
# Available agents: engine | gameplay | ui | qa
#
# Each agent runs in its own named tmux window:
#   🎯orch, 🔧engine, 🎮play, 🎨ui, ✅qa
#
# Navigate: Ctrl+b <number>  or  Ctrl+b n/p (next/prev)
#
# ── Sending tasks to agents ──────────────────────────────────────────────────
#   ./scripts/send-task.sh engine   "implement the bitECS world + components per SPEC §4.2"
#   ./scripts/send-task.sh gameplay "add Stormcloud chain-lightning tower (SPEC §6.1)"
#   ./scripts/send-task.sh ui       "build the TowerPicker HUD component"
...
send_task() {
  local window="$1"
  local message="$2"
  tmux send-keys -t "bloom-agents:${window}" "$message" Enter
  sleep 0.2
  tmux send-keys -t "bloom-agents:${window}" "" Enter
}
...
agent_status() {
  ...
  while IFS=: read -r idx name; do
    ...
    if echo "$pane_text" | grep -q "esc to interrupt"; then
      local activity
      activity=$(echo "$pane_text" | grep -E "^\s*(Reading|Writing|Searching|Running|Booting|⏺)" | tail -1 | sed 's/^[[:space:]]*//' | cut -c1-50)
...
ROOT="$HOME/github/bloom-bastion"
PROFILES="$HOME/.claude/profiles"

# ── Validate args ─────────────────────────────────────────────────────────────
VALID_AGENTS=("engine" "gameplay" "ui" "qa")
AGENTS=()
...
for arg in "$@"; do
  ...
  if $valid; then
    ...
    echo "Available: engine | gameplay | ui | qa"
    exit 1
  fi
done

# ── Agent working directory ───────────────────────────────────────────────────
# Bloom Bastion is a single package (one package.json at root), so every agent
# works from the repo root and scopes its edits to its own src/ subtree.
agent_cwd() {
  echo "$ROOT"
}
...
agent_tab_rgb() {
  # Bloom Bastion palette-inspired dark tab backgrounds
  case "$1" in
    orchestrator) echo "15 14 46" ;;   # bg-deep (deep night violet)
    engine)       echo "18 40 70" ;;   # sapphire
    gameplay)     echo "45 27 78" ;;   # bg-warm (muted purple)
    ui)           echo "60 30 50" ;;   # blossom-tinted
    qa)           echo "20 59 25" ;;   # success green
    *)            echo "40 40 40" ;;
  esac
}
...
tmux send-keys -t "$SESSION:$(agent_window_name orchestrator)" \
  "claude --dangerously-skip-permissions --append-system-prompt-file \"$PROFILES/bloom-orchestrator.md\"" Enter
...
for agent in "${AGENTS[@]}"; do
  ...
  tmux send-keys -t "$SESSION:$wname" \
    "claude --dangerously-skip-permissions --append-system-prompt-file \"$PROFILES/bloom-${agent}-agent.md\"" Enter
done
